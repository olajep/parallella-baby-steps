...
int my_e_load(const char *executable, e_epiphany_t *dev,
		   unsigned row, unsigned col,
		   e_bool_t start)
{
	return my_e_load_group(executable, dev, row, col, 1, 1, start);
}
...
int my_e_default_load_group(const char *executable, e_epiphany_t *dev,
						  unsigned row, unsigned col,
						  unsigned rows, unsigned cols);
int my_e_load_group(const char *executable, e_epiphany_t *dev,
				 unsigned row, unsigned col,
				 ...
				 e_bool_t start)
{
	if (my_e_default_load_group(executable, dev, row, col, rows, cols))
		return E_ERR;
	...
	for (ihdr = 0; ihdr < ehdr->e_phnum; ihdr++) {
		...
		if (ee_esim_target_p()) {
			...
			memcpy((void *) dst, &src[phdr[ihdr].p_offset],
				   phdr[ihdr].p_filesz);
			usleep(100000);
			// assume writes to epiphany cores have propagated now
			if (memcmp((void *) dst, &src[phdr[ihdr].p_offset], phdr[ihdr].p_filesz)) {
				printf("error at ihdr=%d\n", ihdr);
			}
		}
