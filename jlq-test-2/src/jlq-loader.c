...
int je_load(const char *executable, e_epiphany_t *dev, unsigned row, unsigned col, e_bool_t start)
{
	int status;

	status = je_load_group(executable, dev, row, col, 1, 1, start);
...
int je_load_group(const char *executable, e_epiphany_t *dev, unsigned row, unsigned col, unsigned rows, unsigned cols, e_bool_t start)
{
	e_mem_t      emem;
...
e_loader_diag_t je_set_loader_verbosity(e_loader_diag_t verbose)
{
